...
class teaPagination {
    ...
    */
    private function get_connection(){
        if (!self::$PDOInstance) {
            try {
                $host = (!isset($this->options['connect_db']['host']))? 'localhost' : $this->options['connect_db']['host']; 
                self::$PDOInstance = new PDO('mysql:host='.$host.';dbname=' . $this->options['connect_db']['database'], $this->options['connect_db']['user'], $this->options['connect_db']['password'], array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8"));
